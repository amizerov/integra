...
export default function UserEditClient({ user }: UserEditClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      alert(result.error || 'Ошибка обновления пользователя')
    }
  ...
  const handleDelete = async () => {
    const confirmed = await confirm({
      ...
      message: `Вы уверены, что хотите удалить пользователя "${user.fio}"? Это действие нельзя отменить.`,
      variant: 'danger',
      confirmText: 'Удалить',
    })
